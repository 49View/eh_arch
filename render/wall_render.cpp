...
//    }
    ...
    GeomSPContainer make3dGeometry( SceneGraph& sg, GeomSP eRootH, const V3f& _pos, const std::vector<ArchSegment>& wss, const MaterialAndColorProperty &wallMaterial ) {
        GeomSPContainer ret;
        ...
        for ( const auto& [mat, quads] : wallQuads ) {
            auto mainWall = sg.GB<GT::Mesh>(quads, eRootH, _pos, mat, mapping, GT::Tag(ArchType::WallT));
            ret.emplace_back(mainWall);
        }
        return ret;
    }
